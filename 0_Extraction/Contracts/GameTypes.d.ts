import { ConnectionState, HouseCardState, PlayerActionType, possibleHouseCards, possibleLocations, ReplacementReason } from "./GameConstants.ts";
...
interface IRegionSnapshot {
  id: GameLocation;
  units?: UnitState[];
  garrison?: number;
  controlPowerToken?: Factions;
...
type GameLogData =
  ...
  | WesterosCardExecuted
  | WesterosCardDrawn
  | CombatResult
  | WildlingCardRevealed
  | WildlingBidding
  | HighestBidderChosen
  | LowestBidderChosen
  | PlayerMustered
  | WinnerDeclared
  | RavenHolderWildlingCardPutBottom
  | RavenHolderWildlingCardPutTop
  | RavenHolderReplaceOrder
  | RavenNotUsed
  | RaidDone
  | DarkWingsDarkWordsChoice
  | PutToTheSwordChoice
  | AThroneOfBladesChoice
  | TheBurdenOfPowerChoice
  | WinterIsComing
  | WesterosPhaseBegan
  | ClaimVassalsBegan
  | CombatHouseCardChosen
  | CombatValyrianSwordUsed
  | ClashOfKingsBiddingDone
  | ClashOfKingsFinalOrdering
  | ActionPhaseBegan
  | ActionPhaseResolveRaidBegan
  | ActionPhaseResolveMarchBegan
  ...
  | HouseCardAbilityNotUsed
  | PatchfaceUsed
  | DoranUsed
  | TyrionLannisterHouseCardReplaced
  | TyrionLannisterChoiceMade
  ...
  | QueenOfThornsNoOrderAvailable
  | RenlyBaratheonNoFootmanAvailable
  | RenlyBaratheonNoKnightAvailable
  | RenlyBaratheonFootmanUpgradedToKnight
  | MaceTyrellNoFootmanAvailable
  | MaceTyrellCasualtiesPrevented
  | MaceTyrellFootmanKilled
  | CerseiLannisterNoOrderAvailable
  | CerseiLannisterOrderRemoved
  | RobbStarkRetreatRegionOverriden
  | RetreatRegionChosen
  | RetreatCasualtiesSuffered
  | RetreatFailed
  | SilenceAtTheWallExecuted
  | PreemptiveRaidChoiceDone
  | PreemptiveRaidTrackReduced
  | PreemptiveRaidUnitsKilled
  | PreemptiveRaidWildlingsAttack
  | MassingOnTheMilkwaterHouseCardsBack
  | MassingOnTheMilkwaterWildlingVictory
  ...
  | SpymasterExecuted
  | WesterosDeck4Skipped
  | ObjectivesChosen
  | NewObjectiveCardDrawn
  | SpecialObjectiveScored
  | ObjectiveScored
  | IronbornRaid
  | ShiftingAmbitionsObjectiveChosenFromHand
  | ShiftingAmbitionsObjectiveChosenFromPool
  | NewInformationObjectiveCardChosen
  | RevealAllObjectives
  | GarrisonRemoved
  | GarrisonReturned
  | ObjectiveDeckEmpty
...
type BidTracks = "Iron Throne" | "Fiefdom" | "King's Court"
type Factions = "baratheon" | "tyrell" | "lannister" | "arryn" | "greyjoy" | "targaryen" | "martell" | "stark" | "unknown"
...
interface SupportAttackAgainstNeutralForce {
...
interface WesterosDeck4Skipped {
  type: "westeros-deck-4-skipped";
  westerosCardType?: string;
  reason: "defeated" | "vassalized";
}

interface NoLoyaltyTokenAvailable {
  type: "no-loyalty-token-available";
  region: string;
}

interface LastLandUnitTransformedToDragon {
...
export default class GameReplayManager {
  ...
  selectedSnapshot: EntireGameSnapshot | null;
  highlightHouseAreas: boolean;

  castleRegions: Map<string, any>;
  landRegions: Map<string, any>;

  highlighter: any;

  seenSnapshots: Map<number, EntireGameSnapshot>;
  entireGame: EntireGame;
  migrator: any;
  ...
  isReplacementGameLog(log: GameLogData): boolean;
}
...
declare class SnapshotMigrator {
  constructor(ingame: IngameGameState);
  applyLogEvent(snap: EntireGameSnapshot, log: GameLogData, gameLogIndex: number): EntireGameSnapshot;
  handleVassalReplacement(snap: EntireGameSnapshot, log: GameLogData): EntireGameSnapshot;
  resetCombatLogData(): void;
}

interface IEntireGameSnapshot {
  worldSnapshot: IRegionSnapshot[];
  gameSnapshot?: IGameSnapshot;
}

declare class EntireGameSnapshot {
  worldSnapshot: IRegionSnapshot[];
  gameSnapshot?: IGameSnapshot;

  private ingame: IngameGameState;
  private _regionMap: Map<string, IRegionSnapshot>;
  private _houseMap: Map<string, IHouseSnapshot>;
  private _controllerMap: Map<IRegionSnapshot, IHouseSnapshot | null>;

  constructor(data: IEntireGameSnapshot, ingame: IngameGameState);

  getCopy(): EntireGameSnapshot;
  calculateControllersPerRegion(): void;
  getController(regionId: string): IHouseSnapshot | null;
  ...
  getVictoryTrack(): IHouseSnapshot[];
  ...
  private getVictoryConditions(isLastRound: boolean): ((h: IHouseSnapshot) => number)[];
