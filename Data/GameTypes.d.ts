...
type Factions = "baratheon" | "tyrell" | "lannister" | "arryn" | "greyjoy" | "targaryen" | "martell" | "stark"
...
interface EntireGameSnapshot {
  ...
  constructor(data: { worldSnapshot: IRegionSnapshot[]; gameSnapshot?: IGameSnapshot }, ingame: IngameGameState);
  ...
  getVictoryPoints(house: string): number;

  getCastleCount(houseId: string): number;

  getLandRegionCount(houseId: string): number;

  getVictoryConditions(isLastRound: boolean): ((h: IHouseSnapshot) => number)[];

  loadHouseDataFromGame(): void;

  determineController(regionId: string): string | null;
}
